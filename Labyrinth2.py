...
def build_grid(x, y, w):
    for i in range(1,21):
        ...
        for j in range(1, 21):
            pygame.draw.line(screen, BLACK, [x, y], [x + w, y], 2)           # top of cell
            pygame.draw.line(screen, BLACK, [x + w, y], [x + w, y + w], 2)   # right of cell
            pygame.draw.line(screen, BLACK, [x + w, y + w], [x, y + w], 2)   # bottom of cell
            pygame.draw.line(screen, BLACK, [x, y + w], [x, y], 2)           # left of cell
            grid.append((x,y))                                            # add cell to grid list
            x = x + 20                                                    # move cell to new position


def push_up(x, y):
    pygame.draw.rect(screen, RED, (x + 1, y - w + 1, 18, 38), 0)         # draw a rectangle twice the width of the cell
    pygame.display.update()                                              # to animate the wall being removed


def push_down(x, y):
    pygame.draw.rect(screen, BLUE, (x +  1, y + 1, 18, 38), 0)
    pygame.display.update()


def push_left(x, y):
    pygame.draw.rect(screen, GREEN, (x - w +1, y +1, 38, 18), 0)
    pygame.display.update()


def push_right(x, y):
    pygame.draw.rect(screen, YELLOW, (x +1, y +1, 38, 18), 0)
    pygame.display.update()


def single_cell(x, y):
    pygame.draw.rect(screen, BLACK, (x +1, y +1, 18, 18), 0)          # draw a single width cell
    pygame.display.update()
...
def solution_cell(x,y):
    pygame.draw.rect(screen, WHITE, (x+8, y+8, 5, 5), 1)             # used to show the solution
    pygame.display.update()                                        # has visited cell
